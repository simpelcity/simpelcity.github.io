...
export default function ProjectGrid({ langIcons }: ProjectGridProps) {
  const username = "simpelcity";
  const filterInput = document.querySelector(".filter-repos")!;
  const { repos, loading, error } = useUserRepos();
  ...
  filterInput.addEventListener("input", (e: Event) => {
    ...
    for (const repo of repos) {
      const repoElement = repo as HTMLElement;
      const repoTitle = repoElement.innerText.split("\n")[0].toLowerCase();
      if (repoTitle.includes(searchLowerText)) {
        repoElement.parentElement?.classList.remove("hide");
      } else {
        repoElement.parentElement?.classList.add("hide");
      }
    }
  })
    ...
    <>
      {
        repos.map((repo) => (
          <Col key={repo.id} xs={12} md={6} xl={4}>
            <Card className="h-100 rounded-0 border-0 shadow bg-dark-subtle text-primary repo-card text-center">
